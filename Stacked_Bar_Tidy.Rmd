...
# Split Rhizopogon by species for EMF
physeq_emf_melt$genus_species <- ifelse(
  physeq_emf_melt$genus == "Rhizopogon",
  paste0("Rhizopogon_", physeq_emf_melt$species),
  physeq_emf_melt$genus
)
...
# Assign black outline only to Rhizopogon species bars
physeq_melt$border_color <- ifelse(
  grepl("^Rhizopogon_", physeq_melt$genus_species),
  "black",
  NA_character_
)
...
# Black outlines only for Rhizopogon species (keeps internal separators)
physeq_melt$rhizo_border <- ifelse(grepl("^Rhizopogon_", physeq_melt$genus_species), "black", NA)

# Plot 
## Figure 6: All Taxa Stacked Bar
all_taxa_plot <- ggplot(physeq_melt, aes(Sample, Abundance)) +
  geom_bar(stat="identity", position="stack", width=0.9,
           aes(fill = genus_species_legend, color = rhizo_border),
           linewidth = 0.1, show.legend = TRUE) +
  scale_fill_manual(values = all_taxa_colors_fixed, drop = FALSE) +
  scale_color_identity(guide = "none") +
  facet_grid(~ burn_severity, scales = "free_x", space = "free") +
...
# Assign black outline only to Rhizopogon species
physeq_emf_melt$border_color <- ifelse(
  grepl("^Rhizopogon_", physeq_emf_melt$genus_species),
  "black",
  NA_character_
)
...
emf_labels[grepl("^Rhizopogon_", emf_labels)] <- NA
names(emf_colors) <- emf_labels
...
emf_taxa_plot <- ggplot(physeq_emf_melt, aes(x = Sample, y = Abundance)) +
  ...
  scale_color_manual(values = c("black" = "black", "NA" = NA), guide = "none") +
  facet_grid(~burn_severity, scales = "free_x", space = "free") +
  theme_minimal(base_size = 14) +
  theme(
