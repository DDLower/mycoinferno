...
burn_colors_cb <- c(
"High" = hcl(15, 75, 55),
"Low (New)" = hcl(260, 60, 65),
"Low (Old)" = hcl(75, 65, 75),
"Never" = hcl(330, 50, 70)
)

# CLD
unsterilized_cld$burn <- recode(unsterilized_cld$burn, h = "High", ln = "Low (New)", lo = "Low (Old)", n = "Never")
unsterilized_cld$label_y <- max(biomass_unsterilized$dry_biomass, na.rm = TRUE) + 0.05

# Plot unsterilized soil biomass
final_burn_plot <- biomass_unsterilized %>%
mutate(burn = recode(burn, h = "High", ln = "Low (New)", lo = "Low (Old)", n = "Never")) %>%
...
geom_text(data = unsterilized_cld, aes(x = burn, y = label_y, label = .group), inherit.aes = FALSE, fontface = "bold", size = 5) +
...
burn_cld_sterilized$burn <- recode(burn_cld_sterilized$burn, h = "High", ln = "Low (New)", lo = "Low (Old)", n = "Never")
burn_cld_sterilized$label_y <- max(biomass_sterilized$dry_biomass, na.rm = TRUE) + 0.05
...
geom_jitter(width = 0.15, size = 1.5, shape = 21, color = "black", stroke = 0.3, alpha = 0.6) +
geom_text(data = burn_cld_sterilized, aes(x = burn, y = label_y, label = .group), inherit.aes = FALSE, fontface = "bold", size = 5) +
scale_fill_manual(values = burn_colors_cb) +
...
```{r combined-plot}
# Figure 1: Biomass
combined_biomass_plot <- final_burn_plot + plot_sterilized + plot_layout(nrow = 1, guides = "collect")
