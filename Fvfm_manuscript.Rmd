...
## **4. Plot**
```{r}
# Clean and re-level burn severity labels
fvfm_data_clean$Water <- gsub("_", " ", fvfm_data_clean$Water)
fvfm_data_clean$Burn_Severity <- gsub("_", " ", fvfm_data_clean$Burn_Severity)
fvfm_data_clean$Burn_Severity <- case_when(
  fvfm_data_clean$Burn_Severity == "High"     ~ "High",
  fvfm_data_clean$Burn_Severity == "Never"    ~ "Never",
  fvfm_data_clean$Burn_Severity == "Low Old"  ~ "Low (Old)",
  fvfm_data_clean$Burn_Severity == "Low New"  ~ "Low (New)",
  TRUE                                   ~ NA_character_
)
fvfm_data_clean$Burn_Severity <- factor(fvfm_data_clean$Burn_Severity,
                                  levels = c("High", "Low (New)", "Low (Old)", "Never"))

# Ensure correct factor levels for facets
fvfm_data_clean$Sterilization <- factor(fvfm_data_clean$Sterilization,
                                  levels = c("Sterilized", "Unsterilized"))

# Colorblind-friendly palette
cb_palette <- c("ABA" = "#E69F00", "Water Limitation" = "#009E73", "Moist" = "#56B4E9")

# Add daggers to show significant difference between sterilized and unsterilized panels
